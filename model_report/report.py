...
class ReportAdmin(object):
    ...
    def __init__(self, parent_report=None, request=None):
                ...
                if '__' in field:  # IF field has lookup
                    ...
                    for field_lookup in field.split("__"):
                        if not pre_field:
                            ...
                            if 'ManyToManyField' in unicode(pre_field) or isinstance(pre_field, ForeignObjectRel):
        ...
        if parent_report:
            ...
            self.related_inline_field = [f for f, x in fields_w_model if f.rel and hasattr(f.rel, 'to') and f.rel.to is self.parent_report.model][0]
    ...
    def render(self, request, extra_context={}):
        ...
        return render_to_response('model_report/report.html', context_or_response)
